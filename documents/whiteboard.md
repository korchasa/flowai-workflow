...
- [ ] Определён порядок реализации stages
- [ ] Определён формат "ручного" выполнения stages (пока pipeline не готов)
- [ ] Определены issues для первого прогона pipeline
- [ ] Определён критерий "pipeline работает" (переход на полный автомат)
- [ ] План зафиксирован в whiteboard, одобрен пользователем
